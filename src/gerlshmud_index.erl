...
get(Pid) when is_pid(Pid) ->
    gen_server:call(gerlshmud_index, {get, Pid}).

get_pid(Id) when is_atom(Id) ->
    gen_server:call(gerlshmud_index, {get_pid, Id}).

get_id(Pid) when is_pid(Pid) ->
    gen_server:call(gerlshmud_index, {get_id, Pid}).

get_icon(Pid) when is_pid(Pid) ->
    gen_server:call(gerlshmud_index, {get_icon, Pid}).

del(Pid) when is_pid(Pid) ->
    gen_server:cast(gerlshmud_index, {del, Pid}).
